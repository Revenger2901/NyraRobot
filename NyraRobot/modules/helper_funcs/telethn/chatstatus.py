...
from NyraRobot import DRAGONS
from NyraRobot.modules.helper_funcs.telethn import IMMUNE_USERS, telethn
...
async def nyra_is_admin(chat_id: int):
    status = False
    nyra = await telethn.get_me()
    async for user in telethn.iter_participants(
        chat_id, filter=ChannelParticipantsAdmins
    ):
        if nyra.id == user.id:
